...
func _ready():
	OS.set_window_size(Vector2(900,529))
	randomize()
	build_level()
...
#Place end ladder
	...
	var ladder_x = end_room.position.x + 1 + randi() % int(start_room.size.x - 2)
	var ladder_y = end_room.position.y + 1 + randi() % int(start_room.size.y - 2)
